...
export const ProductForm = ({ title, subtitle, product, onSubmit, isPosting }: Props) => {
    ...
    const addTag = () => {
        ...
        const setTag = new Set(getValues('tags'));
        setTag.add(newTag)
        setValue('tags', Array.from(setTag))
    };

    const removeTag = (tagToRemove: string) => {
        const setTag = new Set(getValues('tags'));
        setTag.delete(tagToRemove)
        setValue('tags', Array.from(setTag))
    };
    ...
    const addSize = (size: Size) => {
        ...
        //     }));
        // }
        // Set crea un arreglo, la diferncia es que si el elemento
        // existe, sus elementos son unicos(rebueno!)
        // console.log(size)
        const sizeSet = new Set(getValues('sizes'));
        sizeSet.add(size);
        // se agrega el item a los valores del form
        // se usa Array.from ya que técnicamente
        // SET no es un array aunque se parezca.
        setValue('sizes', Array.from(sizeSet))
    };

    const removeSize = (sizeToRemove: Size) => {

        const sizeRemove = new Set(getValues('sizes'));
        sizeRemove.delete(sizeToRemove)
        setValue('sizes', Array.from(sizeRemove))
        // setProduct((prev) => ({
        //     ...prev,
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div>
                                            ...
                                            />
                                            {errors.price && <p className="text-red-500 text-sm">Por favor verifique el precio ingresado.</p>}
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            />
                                            {errors.stock && <p className="text-red-500 text-sm">Por favor verifique el stock ingresado.</p>}
                                        </div>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        />
                                        {errors.description && <p className="text-red-500 text-sm">Por favor ingrese una descripciom.</p>}
                                    </div>
                                </div>
                            </div>
                                        ...
                                        {availableSizes.map((size) => (
                                            <span
                                                ...
                                                <button
                                                    ...
                                                    className={cn("cursor-pointer ml-2 text-blue-600 hover:text-blue-800 transition-colors duration-200")}
                                                >
                                                    <X className="h-3 w-3" />
                                                </button>
                                            </span>
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="bg-white rounded-xl shadow-lg border border-slate-200 p-6">
                                ...
                                {
                                    files.length === 0 && (<div>No existen archivos</div>)
                                }
